...
public class MongoRepository {
    ...
    public boolean tryLock() {
        ...
        try {
            ...
            if (ex.getError().getCategory() == DUPLICATE_KEY) {
                Document lock = collection.find(eq("_id", LOCK_ID)).first();
    ...
    public void releaseLock() {
        ...
        database.getCollection(CHANGELOG_LOCK_COLLECTION).deleteOne(eq("_id", LOCK_ID));
    }

    /**
     * Runs a command using pessimistic lock in order to ensure no parallel executions of the command can happen.
     *
     * @param command the command to execute
     * @return true, if the lock could be accquired and the command was executed
     */
